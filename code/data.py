...
class Tree(object):
    ...
    class Node(object):
        ...
        def __init__(self, part_id=0, is_leaf=False, box=None, label=None, children=None, edges=None, full_label=None, geo=None, geo_feat=None,img =None):
            ...
            self.img = img                  # 3 x 224 x 224 img
...
class PartNetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if 'object' in self.data_features:
            obj = self.load_object(os.path.join(self.root.replace('_geo','_hier_3000'), self.object_names[index]+'.json'), \
    ...
    @staticmethod
    def load_object(fn, load_geo=False, load_image=True):
        # print(fn)
        if load_geo:
        ...
        if load_image:
            img_fn = fn.replace('_hier', '_img').replace('json', 'npz')
            img_data = np.load(img_fn)
        ...
        if load_image:
            root.image = torch.tensor(img_data['image'], dtype=torch.float32).permute(0, 3, 1, 2)
        obj = Tree(root=root)
...
class PartNetImgDataset(data.Dataset):
    ...
    def load_object(fn, load_geo=False, load_image=True):
        ...
        if load_geo:
            geo_fn = fn.replace('_hier', '_geo').replace('json', 'npz')
            geo_data = np.load(geo_fn)
            for index in range(geo_data['parts'].shape[0]):
                geo_data['parts'][index,:] = normal_pc(geo_data['parts'][index,:])
        ...
        while len(stack) > 0:
            ...
            if load_image:
                node.img = (torch.tensor(img_data['image'][node_json['id']], dtype=torch.float32)/255).unsqueeze(0).permute(0, 3, 1, 2)
    ...
    def save_object(obj, fn):
        ...
        while len(stack) > 0:
            ...
            if node.img is not None:
                node_json['img'] = node.img.cpu().numpy().reshape(-1).tolist()
