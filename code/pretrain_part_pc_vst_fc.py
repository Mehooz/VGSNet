...
def train(conf):
    ...
    conf.exp_name = 'part_pc_ae_chair_3000_vst_fc'
    conf.category = 'Chair'
    conf.data_path = '../data/partnetdata/chair_geo_3000'
    conf.train_dataset = 'train_no_other_less_than_10_parts.txt'
    conf.val_dataset = 'val_no_other_less_than_10_parts.txt'
    conf.epochs = 1000
    conf.model_version = 'model_part_vst_fc'
    conf.batch_size = 1
    # conf.lr = 1e-3
    conf.lr = 1e-4
...
def forward(batch, encoder, decoder,device, conf,
    ...
    net,kldiv_loss = encoder(pts,imgs)

    if not conf.non_variational:
        # fea, kldiv_loss = torch.chunk(net, 2, 1)
    ...
    total_loss = cd_loss + mse_loss*0.5  + kldiv_loss * conf.loss_weight_kldiv
