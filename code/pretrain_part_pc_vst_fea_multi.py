...
def weights_init(m):
    ...
    if classname.find('Linear') != -1:
        torch.nn.init.uniform_(m.weight.data,a = -0.2,b =0.2)
        torch.nn.init.uniform_(m.bias.data,a = -0.2,b =0.2)


def train(conf):
    # load network model
    # os.environ['CUDA_VISIBLE_DEVICES']='2'
    conf.exp_name = 'part_pc_ae_chair_3000_vst_fea_kl0.01_multi_2_parts_img_encoder'
    conf.category = 'Chair'
    conf.data_path = '/repository/zhangxc1/chair_geo_new'
    conf.train_dataset = 'train_no_other_less_than_10_parts.txt'
    conf.val_dataset = 'test_no_other_less_than_10_parts.txt'
    conf.epochs = 100
    conf.model_version = 'model_part_vst_fea'
    conf.batch_size = 1
    # conf.lr = 1e-3
    conf.lr = 1e-4
    conf.lr_decay_every = 3000
    conf.lr_decay_by = 0.9
    conf.device = 'cuda:0'
    conf.num_point = 3000
    conf.non_variational = False
    conf.checkpoint_interval = 20000
    conf.loss_weight_kldiv = 0.01
    conf.start_epoch = 0
    conf.load_model = True
    conf.model_path='/home/zhangxc/tmp/structurenet-master/data/models'
    ...
    # create models
    encoder = models.PartEncoder(feat_len=256, probabilistic=not conf.non_variational).to(device)
    # encoder = models.PartEncoder(feat_len=conf.geo_feat_size)
    if conf.load_model:
        encoder.load_state_dict(torch.load(f'/home/zhangxc/tmp/structurenet/data/models/part_pc_ae_chair_3000_vst_fea_kl0.01_multi_2/net_part_pc_encoder.pth'))

    img_encoder=ImageEncoder(512)

    models = [img_encoder]
    model_names = ['img_encoder']

    # create optimizers
    img_encoder_opt = torch.optim.Adam(encoder.parameters(), lr=conf.lr, weight_decay=conf.weight_decay)
    optimizers = [img_encoder_opt]
    optimizer_names = ['part_pc_img_encoder']

    # learning rate scheduler
    img_encoder_scheduler = torch.optim.lr_scheduler.StepLR(img_encoder_opt, \
    ...
    for epoch in range(conf.start_epoch+1,conf.start_epoch+conf.epochs):
        if not conf.no_console_log:
        ...
        for train_batch_ind, batch in train_batches:
            ...
            total_loss = forward(
                batch=batch, encoder=encoder, img_encoder=img_encoder,device=device, conf=conf,
                ...
                lr=img_encoder_opt.param_groups[0]['lr'], flog=flog)

            # optimize one step
            img_encoder_scheduler.step()
            img_encoder_opt.zero_grad()
            total_loss.backward()
            img_encoder_opt.step()
...
def forward(batch, encoder, img_encoder,device, conf,
            is_valdt=False, step=None, epoch=None, batch_ind=0, num_batch=1, start_time=0,
            log_console=False, log_tb=False, tb_writer=None, lr=None, flog=None,loss=nn.MSELoss()):
    ...
    gt = encoder(pts,imgs)[0:4,0:512]
    pred=img_encoder(imgs)

    total_loss = loss(gt,pred)
