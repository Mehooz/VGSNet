...
class PartNetGeoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = torch.tensor(img_set[idx,:,:,:],dtype = torch.float32)
...
def train(conf):
    # load network model
    conf.exp_name = 'part_pc_ae_chair_3000_img'
    conf.category = 'Chair'
    conf.data_path = '/home/zhangxc/tmp/structurenet/data/partnetdata/chair_geo_3000'
    conf.train_dataset = 'train_no_other_less_than_10_parts_tiny.txt'
    conf.val_dataset = 'test_no_other_less_than_10_parts_tiny.txt'
    conf.epochs = 200

    conf.model_version = 'model_part_pc_img'
    conf.batch_size = 1
    conf.lr = 1e-3
    conf.lr_decay_every = 300
    ...
    decoder = models.PartImgDecoder(feat_len=conf.geo_feat_size, num_point=conf.num_point)
    ...
    grid_dist_tensor=grid_dist(64,64)
...
def forward(batch, encoder, decoder,device, conf,
    ...
    pred = decoder(net,imgs)
    recon_loss = decoder.loss(pred, pts,4,pred.shape[0],conf.device,3000)
