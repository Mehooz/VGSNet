...
def load_checkpoint(models, model_names, dirname, epoch=None, optimizers=None, optimizer_names=None, strict=True):
    ...
    for model, model_name in zip(models, model_names):
        ...
        pretrained_dict=torch.load(os.path.join(dirname, filename))
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and k is not 'node_decoder'}
